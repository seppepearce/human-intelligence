  ...
  const friends = writable([
    {
      ...
      name: "Plato",
      avatar: "📚",
      currentSpace: "Philosophy of Mind",
      status: "active",
    },
    {
      id: "friend_2",
      ...
      spaces.update((currentSpaces) => {
        return currentSpaces.map((space) => {
          ...
          const newParticipants = Math.max(
            1,
            Math.min(space.capacity, space.participants + change),
          );
...
<svelte:head>
  ...
  <meta
    name="description"
    content="Welcome to the Digital Agora - where meaningful discourse flows like ancient streams of wisdom."
  />
</svelte:head>
...
>
  ...
  <header
    ...
    style="border-color: rgba(212, 175, 55, 0.2); background-color: rgba(26, 26, 46, 0.5);"
  >
    <div class="max-w-7xl mx-auto px-6 py-4">
      <div class="flex items-center justify-between">
        <div class="flex items-center space-x-3">
          ...
          <div>
            <h1
              ...
              style="color: #d4af37; text-shadow: 0 0 10px rgba(212, 175, 55, 0.3);"
            >
              Digital Agora
            </h1>
            <p class="text-sm" style="color: rgba(233, 236, 239, 0.7);">
              The Plaza of Ideas
            </p>
          </div>
        </div>
        ...
        <div class="flex items-center space-x-4">
          <div class="text-sm" style="color: rgba(233, 236, 239, 0.7);">
            Welcome, <span class="font-medium" style="color: #d4af37;"
  ...
  <div class="max-w-7xl mx-auto px-6 py-8">
    ...
    <div class="mb-8">
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div
          ...
          style="background-color: rgba(22, 33, 62, 0.5); border-color: rgba(212, 175, 55, 0.2);"
        >
          <div class="text-2xl font-bold" style="color: #d4af37;">
            {$spaces.reduce((sum, s) => sum + s.participants, 0)}
          </div>
          <div class="text-sm" style="color: rgba(233, 236, 239, 0.7);">
            Active Participants
          </div>
        ...
        >
          <div class="text-2xl font-bold" style="color: #d4af37;">
            {$spaces.filter((s) => s.status === "full").length}
          </div>
          <div class="text-sm" style="color: rgba(233, 236, 239, 0.7);">
            Crowded Spaces
          </div>
    ...
    {#if $friends.some((f) => f.status === "active")}
      <div class="mb-8">
        <h3
          class="text-lg font-semibold text-agora-gold mb-4 flex items-center"
        >
        ...
        <div class="flex flex-wrap gap-3">
          {#each $friends.filter((f) => f.status === "active") as friend}
            <div
              class="bg-agora-deep/30 rounded-full px-4 py-2 border border-agora-gold/20 flex items-center space-x-2"
            >
              <span class="text-lg">{friend.avatar}</span>
              <span class="text-sm text-agora-stone"
                >{friend.name} is exploring</span
              >
              <span class="text-sm font-medium text-agora-gold"
                >{friend.currentSpace}</span
              >
              <button
                class="text-xs text-agora-neon-cyan hover:text-agora-neon-purple transition-colors"
                on:click={() =>
                  joinSpace(
                    friend.currentSpace.toLowerCase().replace(/ /g, "-"),
                  )}
              >
                Join →
              </button>
    ...
    <div class="mb-8">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {#each $spaces as space}
          <div
            class="bg-agora-deep/50 rounded-lg p-6 border {getActivityColor(
              space.activity,
              space.status === 'full',
            )} hover:border-agora-gold/50 transition-all duration-300 hover:shadow-lg hover:shadow-agora-gold/10"
          >
            <!-- Space Header -->
            <div class="flex items-start justify-between mb-4">
              <div class="flex-1">
                <h4 class="font-semibold text-agora-marble mb-1">
                  {space.name}
                </h4>
                <p class="text-sm text-agora-stone/70 leading-relaxed">
                  {space.description}
                </p>
              </div>
            ...
            <div class="w-full bg-agora-night/50 rounded-full h-2 mb-4">
              <div
                class="h-2 rounded-full transition-all duration-300 {getCapacityBarColor(
                  (space.participants / space.capacity) * 100,
                )}"
            ...
            <!-- Space Stats -->
            <div
              class="flex items-center justify-between text-xs text-agora-stone/60 mb-4"
            >
              <div class="flex items-center space-x-4">
            ...
            <div class="flex flex-wrap gap-2 mb-4">
              {#each space.topics as topic}
                <span
                  class="bg-agora-night/50 text-agora-stone/80 px-2 py-1 rounded-full text-xs"
                >
                  {topic}
                </span>
              {/each}
            ...
            <!-- Action Button -->
            <button
              class="w-full py-2 px-4 rounded-lg font-medium transition-all duration-200 {space.status ===
              'full'
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
      ...
      <div>
        ...
        <div class="space-y-3">
          {#each suggestions.slice(0, 3) as space}
            <div
              class="bg-agora-deep/30 rounded-lg p-4 border border-agora-gold/20 hover:border-agora-gold/40 transition-all"
            >
              <div class="flex items-center justify-between">
                <div class="flex-1">
                  <h5 class="font-medium text-agora-marble">{space.name}</h5>
                  <p class="text-sm text-agora-stone/70">
                    {space.participants}/{space.capacity} participants
                  </p>
                  {#if space.friends.length > 0}
                    <p class="text-xs text-agora-neon-cyan mt-1">
                      Friends: {space.friends.join(", ")}
                    </p>
                  {/if}
                </div>
      ...
      <div>
        ...
        <p class="text-sm text-agora-stone/70 mb-4">
          Venture beyond your usual domains. Great insights often come from
          unexpected connections.
        </p>
        <div class="space-y-3">
          {#each crossPollination as space}
            <div
              class="bg-agora-deep/30 rounded-lg p-4 border border-agora-neon-purple/20 hover:border-agora-neon-purple/40 transition-all"
            >
              <div class="flex items-center justify-between">
                <div class="flex-1">
                  <h5 class="font-medium text-agora-marble">{space.name}</h5>
                  <p class="text-sm text-agora-stone/70">
                    {space.participants}/{space.capacity} participants • {space.avgDepth}/10
                    depth
                  </p>
                  <div class="flex flex-wrap gap-1 mt-2">
                    {#each space.topics.slice(0, 2) as topic}
                      <span
                        class="bg-agora-neon-purple/20 text-agora-neon-purple px-2 py-0.5 rounded-full text-xs"
                      >
                        {topic}
                      </span>
                    {/each}
